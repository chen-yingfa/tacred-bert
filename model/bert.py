...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, e1_pos_seq=None, e2_pos_seq=None):
        ...
        if e1_pos_seq is not None:
            # Input method 2, position embedding
            seq_length = input_ids.size(1)
            num_embeddings = 2 * seq_length
            # print("num_embeddings:", num_embeddings)
            e_pos_embeddings = nn.Embedding(num_embeddings, self.hidden_size)
            e_pos_embeddings = e_pos_embeddings.to(input_ids.device)

            e1_pos_embeds = e_pos_embeddings(e1_pos_seq)
            e2_pos_embeds = e_pos_embeddings(e2_pos_seq)
        
        position_embeddings = self.position_embeddings(position_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        if e1_pos_seq is None:
            # Ordinary embeddings
            embeddings = inputs_embeds + position_embeddings + token_type_embeddings
        else:
            # Input method 2, position embedding
            # print("inputs_embeds.shape:", inputs_embeds.shape)
            # print("e1_pos_embeds.shape:", e1_pos_embeds.shape)
            # print("e2_pos_embeds.shape:", e2_pos_embeds.shape)
            embeddings = inputs_embeds + position_embeddings + token_type_embeddings + e1_pos_embeds + e2_pos_embeds
...
class BertClassifier(nn.Module):
    def __init__(self, pretrain_path, num_labels=42, hidden_size=768):
        super().__init__()
